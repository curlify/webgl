...
(function() {
  ...
  var CanvasInput = window.CanvasInput = function(o) {
    ...
    //self._hiddenInput.style.top = -200;
    self._hiddenInput.style.width = self._width/2 + 'px';
    ...
    self._hiddenInput.oninput = function() {
      //console.log("oninput")
      self.render()
    }
  ...
  CanvasInput.prototype = {
    ...
    keydown: function(e, self) {
      ...
      /*
      // add support for jump to begin
      if (e.metaKey && (keyCode === 37 || keyCode === 38)) {
        self._cursorPos = 0
        return self.render();
      }

      // add support for jump to end
      if (e.metaKey && (keyCode === 39 || keyCode === 40)) {
        self._cursorPos = self._value.length
        return self.render();
      }
      */
      ...
      if (keyCode === 13) { // enter key
        ...
        //console.log(inputs.length,self._inputsIndex,inputs[self._inputsIndex+1])
        e.preventDefault();
        if (inputs.length > 1) {
    ...
    mousemove: function(e, self) {
      ...
      //console.log("mousemove",self._hasFocus,self._selectionStart)
      if (self._hasFocus && self._selectionStart >= 0) {
        ...
        //console.log("trying to update selection",self._selection)
      }
    },
    ...
    render: function() {
      ...
      self._drawTextBox(function() {
        ...
        // clip the text so that it fits within the box - actually don't
        var text = self._clipText();
        text = (self._type === 'password' && text != self._placeHolder ? text.replace(/./g,'*') : text )

        // define selection as what is selected in the real input box
        self._selection = [self._hiddenInput.selectionStart,self._hiddenInput.selectionEnd]
        // if selecting forward, move cursor to end of selection
        if (self._hiddenInput.selectionDirection == 'forward') self._cursorPos = self._hiddenInput.selectionEnd

        // calculate text area width visible for text and then the offset for rendering so cursor is shown
        var textareawidth = (self._width - self._padding*2 - self._borderWidth*2 - self.shadowL)
        var cursorOffset = self._textWidth( text.substring(0, self._cursorPos) )
        var off = cursorOffset > textareawidth ? (textareawidth-cursorOffset) : 0

        var paddingBorder = self._padding + self._borderWidth + self.shadowT;

        // draw the selection
        if (self._hasFocus && self._selection[1] > 0) {
          ...
          ctx.fillRect(paddingBorder + selectOffset+off, paddingBorder, selectWidth, self._height);
        }

        // draw the cursor
        if (self._cursor) {
          ctx.fillStyle = self._fontColor;
          ctx.fillRect(paddingBorder + cursorOffset + off, paddingBorder, 1, self._height);
        }

        // draw the text
        var textX = self._padding + self._borderWidth + self.shadowL + off,
        ...
        if (self._ctx) {
          //console.log("drawimg")
          //console.log("render: '"+self._hiddenInput.value+"'",Date.now())
    ...
    destroy: function() {
      ...
      //console.log("found index",index,inputs.length)
      if (index >= 0) {
      ...
      //console.log("destroy",this,inputs,self._inputsIndex)
      //if (self._inputsIndex >= inputs.length) self._inputsIndex = inputs.length-1
      //inputs[self._inputsIndex].focus();
    },
